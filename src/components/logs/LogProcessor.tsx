...
const LogProcessor: React.FC<LogProcessorProps> = ({ customPages }) => {
  const [activeView, setActiveView] = useState<'all' | 'analytics' | 'patterns' | 'anomalies' | 'performance'>('all');
  ...
  const mockAnalytics = {
    ...
    criticalIssues: 5,
    topPages: [
      { id: '1', name: 'Home', count: 423 },
      { id: '2', name: 'Dashboard', count: 312 },
      { id: '3', name: 'Settings', count: 187 }
    ],
  ...
  const renderViewContent = () => {
    switch (activeView) {
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-blue-800 dark:text-blue-200">{mockAnalytics.totalLogs}</p>
              </div>
              
              <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg">
                <h3 className="text-sm font-medium text-yellow-700 dark:text-yellow-300 mb-1">Error Rate</h3>
                <p className="text-2xl font-bold text-yellow-800 dark:text-yellow-200">{mockAnalytics.errorRate}%</p>
              </div>
              
              <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-lg">
                <h3 className="text-sm font-medium text-red-700 dark:text-red-300 mb-1">Critical Issues</h3>
                <p className="text-2xl font-bold text-red-800 dark:text-red-200">{mockAnalytics.criticalIssues}</p>
              </div>
            </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                ...
                <div className="flex items-center space-x-2 mb-2">
                  <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-4">
                    <div className="bg-blue-500 h-4 rounded-full" style={{ width: `${mockAnalytics.severityDistribution.info}%` }}></div>
                  </div>
                  <span className="text-xs text-gray-600 dark:text-gray-400 whitespace-nowrap">Info {mockAnalytics.severityDistribution.info}%</span>
                </div>
                <div className="flex items-center space-x-2 mb-2">
                  <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-4">
                    <div className="bg-yellow-500 h-4 rounded-full" style={{ width: `${mockAnalytics.severityDistribution.warning}%` }}></div>
                  </div>
                  <span className="text-xs text-gray-600 dark:text-gray-400 whitespace-nowrap">Warning {mockAnalytics.severityDistribution.warning}%</span>
                </div>
                <div className="flex items-center space-x-2 mb-2">
                  <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-4">
                    <div className="bg-red-500 h-4 rounded-full" style={{ width: `${mockAnalytics.severityDistribution.error}%` }}></div>
                  </div>
                  <span className="text-xs text-gray-600 dark:text-gray-400 whitespace-nowrap">Error {mockAnalytics.severityDistribution.error}%</span>
                </div>
                <div className="flex items-center space-x-2 mb-2">
                  <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-4">
                    <div className="bg-green-500 h-4 rounded-full" style={{ width: `${mockAnalytics.severityDistribution.debug}%` }}></div>
                  </div>
                  <span className="text-xs text-gray-600 dark:text-gray-400 whitespace-nowrap">Debug {mockAnalytics.severityDistribution.debug}%</span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-4">
                    <div className="bg-purple-500 h-4 rounded-full" style={{ width: `${mockAnalytics.severityDistribution.critical}%` }}></div>
                  </div>
                  <span className="text-xs text-gray-600 dark:text-gray-400 whitespace-nowrap">Critical {mockAnalytics.severityDistribution.critical}%</span>
                </div>
              </div>
      ...
      case 'all':
      default:
        return <LogViewer customPages={customPages} />;
    }
  };
